...
class Model():
    ...
    def __init__(self, train_input_file, train_target_file,
        ...
        sess = tf.Session()
        sess.run(init)
        self.log_writter_temp = tf.summary.FileWriter('../models/tf-lib/output_couplet/logs', sess.graph)
		#new segement
    ...
    def train(self, epochs, start=0):
        ...
        with self.train_graph.as_default():
            ...
            for step in range(start, epochs):
                ...
                self.log_writter_temp.add_summary(summary, step)
                ...
                if step % self.eval_step == 0:
                    ...
                    self.log_writter_temp.add_summary(eval_summary, step)
                if step % self.save_step == 0:
                    total_loss = 0
